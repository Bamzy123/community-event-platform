...
export async function createEvent(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (!venueId || isNaN(parsedVenueId)) {
    ...
    res.status(500).json({ error: "An unexpected error occurred while creating event." });
  }
}
...
export async function listEvents(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (venueId) {
      whereClause.venueId = Number(venueId);
    }
...
export async function upvoteEvent(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Authentication required." });
      return;
    ...
    console.error("UpvoteEvent error:", error);
    res.status(500).json({ error: "Failed to upvote event." });
  }
}

export async function removeUpvote(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Authentication required." });
      return;
